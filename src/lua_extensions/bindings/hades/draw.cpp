...
namespace lua::hades::draw
{
	...
	void bind(sol::state_view& state, sol::table& lua_ext)
	{
		...
		// Lua API: Function
		// Table: data
		// Name: draw_sanity_check_gmd
		// Param: entry_name: string: A known stock entry (e.g. "HecateHub_Mesh").
		// Returns: integer — number of failed checks (0 = layout matches).
		// Canary that validates the hardcoded GMD offsets against a live
		// entry.  Call once from first-ImGui-frame with a known entry to
		// catch offset drift after a game update before the visible-
		// symptom ("why is the body invisible?") stage.
		ns.set_function("draw_sanity_check_gmd", [](const std::string& entry) -> int {
			static auto Lookup = *big::hades2_symbol_to_address["sgg::HashGuid::Lookup"]
			    .as_func<sgg::HashGuid*(sgg::HashGuid*, const char*, size_t)>();
			if (!Lookup)
			{
				LOG(ERROR) << "draw_sanity_check_gmd: Lookup missing";
				return 1;
			}
			...
			if (!g.mId)
			{
				LOG(ERROR) << "draw_sanity_check_gmd: entry hash=0 for '" << entry << "'";
				return 1;
			}

			ModelDataNode* node = find_model_data_node(g.mId);
			if (!node)
			{
				LOG(ERROR) << "draw_sanity_check_gmd: entry '" << entry
				           << "' missing from mModelData";
				return 1;
			}

			GrannyMeshData* meshes = nullptr;
			size_t mesh_count = 0;
			if (!get_entry_meshes(node, &meshes, &mesh_count))
			{
				LOG(ERROR) << "draw_sanity_check_gmd: GMD range unreadable or implausible";
				return 1;
			}

			int fails = 0;
			constexpr size_t kMaxMeshesScanned = 20;
			for (size_t i = 0; i < mesh_count && i < kMaxMeshesScanned; i++)
			{
				const GrannyMeshData& gmd = meshes[i];
				if (gmd.mesh_type > 2)
				{
					LOG(ERROR) << "draw_sanity_check_gmd: mesh[" << i
					           << "] type=" << (int)gmd.mesh_type << " (expected 0/1/2)";
					fails++;
				}
				uint32_t mh = 0;
				if (safe_read_u32(&gmd.mesh_name_hash, &mh) && mh == 0)
				{
					LOG(ERROR) << "draw_sanity_check_gmd: mesh[" << i << "] name hash=0";
					fails++;
				}
			}
			if (fails == 0)
				LOG(INFO) << "draw_sanity_check_gmd: '" << entry
				          << "' layout OK (" << mesh_count << " meshes)";
			return fails;
		});
